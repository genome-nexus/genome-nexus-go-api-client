...
For using other server than the one defined on index 0 set context value `genome_nexus_public_api.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `genome_nexus_public_api.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `genome_nexus_public_api.ContextOperationServerIndices` and `genome_nexus_public_api.ContextOperationServerVariables` context maps.
...
## Documentation For Models
 ...
 - [TranscriptConsequenceSummaryAlphaMissense](docs/TranscriptConsequenceSummaryAlphaMissense.md)
